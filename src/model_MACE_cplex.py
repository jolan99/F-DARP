...
def model_MACE_cplex(scenario: Scenario, time_limit : int, verbose: int) -> Solution:
    """ Il s'agit du modèle : modèle 7 + max emissions évitées méthode 1 
            - 4 rôles 
            - coeff devant nb de requests : 9 
            - bonus : nouveau multiplicateur : 1/EP 
            - temps continu 
            - 7 variables
            - obj : max nb requests + max avoided carbon emissions method 1
            
            A besoin des scenarios D """
    model = Model("flexible_carpooling_cplex")

    n = len(scenario.all_requests)
    # s_len = len(scenario.all_stops)
    s_len = len(scenario.dist_matrix_km)
    
    ## Setting bonus value multiplicator
    ...
    # print(f'debug model10_cplex : multiplicator_bonus : {multiplicator_bonus}')
    ## Big-M values
    M1 = 288
    ...
    # )
    # print(f' scenario.dist_matrix_km[s][ss] : {scenario.dist_matrix_km}')
    model.maximize(9 * model.sum(x[u][v] for u in range(n) for v in range(n)) +
                   model.sum(S[u] for u in range(n)) +
                    (scenario.total_potential_emissions - model.sum(scenario.all_requests[u].emission_rate_vehicle * (model.sum( model.sum(y[s+1][ss][u] * float(scenario.dist_matrix_km[s][ss]) for s in range(s_len)) for ss in range(s_len)) )for u in range(n)) )/scenario.total_potential_emissions)
    ...
    # model.add_constraint(y[0][2][0] == 1)
    # model.add_constraint(y[3][0][0] == 1)
    ...
    return Solution(status_,scenario,model.objective_value,nb_requests_satisfied,"MACE_CPLEX", x_values, y_values, F_values, theta_a_values, theta_d_values, Q_values, S_values,'cplex',multiplicator_bonus=multiplicator_bonus,time_limit=time_limit,verbose = verbose)
